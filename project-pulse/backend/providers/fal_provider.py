...
async def generate_with_fal(model_id: str, prompt: str, ratio: str = "16:9", image_url: Optional[str] = None, end_image_url: Optional[str] = None, reference_images: Optional[List[str]] = None, mode: str = "t2v"):
    """
    ...
    try:
        ...
        if mode == "r2v" and reference_images:
            if is_kling_new_gen:
                ...
                arguments["elements"] = [{"reference_image_urls": [url]} for url in reference_images]
                # If they passed reference images + an image_url, image_url becomes start_image_url
                if image_url:
                    arguments["start_image_url"] = image_url
            elif "seedance" in model_id:
                arguments["reference_images"] = [{"image_url": url} for url in reference_images]
            else:

...
import {
  Play, Pause, Volume2, VolumeX, Maximize2, Sparkles, Target, Crown,
  ChevronRight, Vote, Award, BarChart3, Info, ShieldCheck,
  CheckCircle2, AlertCircle, Loader2, Zap, ArrowRight,
  TrendingUp, Activity, UserCircle2, Cpu, History, MessageSquareQuote, Tag, Search
} from 'lucide-react';
...
const MOCK_EVALS = [
  {
    id: 1,
    prompt: "A neon-soaked cyberpunk street in Tokyo with rain reflecting off the asphalt, 8k resolution, cinematic lighting.",
    image_url: "https://images.unsplash.com/photo-1545641203-7d072a14e3b2",
    videos: [
      { id: 'a', url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4", model: "Veo" },
      { id: 'b', url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4", model: "Kling" }
    ]
  },
  {
    id: 2,
    prompt: "Extreme close-up of a realistic eye reflecting a galaxy, macro lens, glowing iris.",
    videos: [
      { id: 'a', url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4", model: "Seedance" },
      { id: 'b', url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4", model: "Veo" }
    ]
  }
];
...
export default function LivingArena() {
  ...
  const [votingStep, setVotingStep] = useState(1); // 1: Select Winner, 2: Rate Dimensions
  const [winner, setWinner] = useState<string | null>(null); // 'a' or 'b'
  const [dimScores, setDimScores] = useState<Record<string, number>>({});
  const [justification, setJustification] = useState("");
  const [showStats, setShowStats] = useState(false);
  const [expandedImage, setExpandedImage] = useState<string | null>(null); // To expand preview images
  ...
  const fetchNewPair = async (forcePromptId?: string) => {
    setIsLoading(true);
    try {
      const qs = forcePromptId ? `?prompt_id=${encodeURIComponent(forcePromptId)}` : '';
  ...
  useEffect(() => {
    if (ldap) {
      fetch(`${API_BASE_URL}/api/evaluation/stats?ldap=${ldap}`)
        ...
        .then(data => {
          if (data && data.user) {
            setVotesCount(data.user.total_evals || 0);
          }
        })
        ...
        <div className="relative z-10 w-full max-w-5xl flex border border-white/10 rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-700 bg-[#0b0e14]/80 backdrop-blur-2xl">
          ...
          <div className="w-full md:w-1/2 p-10 lg:p-16 flex flex-col justify-center border-r border-white/10">
            ...
            <form onSubmit={(e) => {
              e.preventDefault();
              if (ldapInput.trim()) {
                setLdap(ldapInput.trim());
              }
            }}>
              <input
                type="text"
                value={ldapInput}
                onChange={(e) => setLdapInput(e.target.value.toLowerCase())}
              ...
              <button
                type="submit"
                disabled={!ldapInput.trim()}
                className="w-full bg-white text-[#020408] font-black uppercase tracking-widest py-4 rounded-2xl hover:bg-gray-200 transition-colors disabled:opacity-50"
              >
                Enter Evaluation
              </button>
            </form>
          ...
          <div className="hidden md:flex flex-col w-1/2 bg-black/40 p-10 lg:p-16">
            <h3 className="text-2xl font-bold text-white flex items-center gap-3 mb-8">
              <Crown className="w-6 h-6 text-yellow-400" /> Top Evaluators
            </h3>
  ...
  const handleSubmitVote = async () => {
    ...
    try {
      ...
      const newHistoryItem = {
        prompt: currentEval.prompt,
        winner: winner,
        scores: dimScores,
        justification: justification,
        modelA: currentEval.variant_a.model_id,
        modelB: currentEval.variant_b.model_id
      };
      ...
      fetchNewPair(); // Always stay on eval
    } catch (err) {
  ...
  if (showStats) {
    return <StatsDashboard history={history} ldap={ldap} onBack={() => setShowStats(false)} globalLeaderboard={globalLeaderboard} />;
  }
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-gray-100 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      <div className="fixed inset-0 -z-10 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
        ...
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                fetchNewPair(searchPromptId);
              }}
              className="ml-8 relative flex items-center group"
            >
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-white font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:bg-white/10 transition-all w-64 placeholder-gray-500"
              />
            </form>
          </div>
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-end mr-1">
                ...
                <span className="text-xs font-black text-indigo-400 font-mono tracking-tighter">{votesCount} VOTES RECORDED</span>
              </div>
              <div className="w-48 h-2 bg-white/5 rounded-full overflow-hidden border border-white/10 shadow-inner">
                <div
                  ...
                  style={{ width: `${(votesCount / 10) * 100}%` }}
                ></div>
              </div>
              {votesCount >= 10 ? (
                <button
                  onClick={() => setShowStats(true)}
                  className="px-4 py-2 ml-2 rounded-xl bg-indigo-500/20 border border-indigo-500/40 flex items-center gap-2 hover:bg-indigo-500/40 transition-all font-bold text-xs uppercase text-indigo-300 shadow-[0_0_15px_rgba(99,102,241,0.3)] animate-pulse"
                >
      ...
      <main className="max-w-screen-2xl mx-auto px-6 pt-32 pb-20">
          ...
          <div className="flex flex-col lg:flex-row items-start gap-8">
            {currentEval && (
              <div className="flex flex-wrap gap-4 shrink-0">
                {currentEval.start_image_url && (
                  <ScenarioReferenceImage url={currentEval.start_image_url} label="Start Frame" onClick={() => setExpandedImage(currentEval.start_image_url ?? null)} />
                )}
                {currentEval.end_image_url && (
                  <ScenarioReferenceImage url={currentEval.end_image_url} label="End Frame" onClick={() => setExpandedImage(currentEval.end_image_url ?? null)} />
                )}
                {currentEval.reference_image_url && !currentEval.reference_images && (
                  <ScenarioReferenceImage url={currentEval.reference_image_url} label="Ref Image" onClick={() => setExpandedImage(currentEval.reference_image_url ?? null)} />
                )}
                {currentEval.reference_images?.map((url: string, i: number) => (
                  <ScenarioReferenceImage key={i} url={url} label={`Ref ${i + 1}`} onClick={() => setExpandedImage(url ?? null)} />
                ))}
                {!currentEval.start_image_url && !currentEval.end_image_url && !currentEval.reference_image_url && (
                  <ScenarioReferenceImage url="https://images.unsplash.com/photo-1545641203-7d072a14e3b2" label="Text Only" />
                )}
              </div>
            )}
            ...
            <div className="flex-1 space-y-4">
              <div className="flex flex-wrap gap-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-widest">
                  <Sparkles className="w-3 h-3" /> Expert Scrutiny Scenario
                </div>

                {currentEval?.categories && currentEval.categories.length > 0 ? (
                  currentEval.categories.map((cat: string) => (
                    <div key={cat} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-widest">
                      <Tag className="w-3 h-3" /> {cat}
                    </div>
                  ))
                ) : currentEval?.category ? (
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-widest">
                    <Tag className="w-3 h-3" /> {currentEval.category}
                  </div>
                ) : null}
              </div>

              <h1 className="text-3xl md:text-5xl font-light text-white leading-[1.15] tracking-tight">
                "{currentEval?.prompt || "Loading Scenario..."}"
              </h1>
              <div className="flex items-center gap-4 text-sm text-gray-500 italic font-light">
                <span className="flex items-center gap-2 font-mono"><UserCircle2 className="w-4 h-4" /> {currentEval?.job_id ? `PID: ${currentEval.job_id}` : "Expert Bench V-7"}</span>
        ...
        <div className="flex justify-center mb-8">
          ...
          >
            ...
            <span className="text-sm font-bold uppercase tracking-widest text-indigo-400">Play Both Simultaneously</span>
          </button>

          <button
            onClick={handleSkip}
            className="flex items-center gap-3 px-8 py-4 bg-red-500/5 border border-red-500/10 rounded-2xl hover:bg-red-500/10 transition-all group"
          >
            <div className="p-2 bg-red-500/20 rounded-lg group-hover:scale-110 transition-transform">
              <ChevronRight className="w-4 h-4 text-red-400" />
            </div>
            <span className="text-sm font-bold uppercase tracking-widest text-red-400">Skip Scenario</span>
          </button>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
          {isLoading ? (
            <div className="lg:col-span-2 py-40 flex flex-col items-center justify-center text-gray-500 gap-4">
            ...
            <div className="lg:col-span-2 py-40 flex flex-col items-center justify-center text-gray-500 gap-4">
              ...
              <p className="font-light tracking-widest uppercase text-xs">No active scenarios found. Check Admin Console.</p>
            </div>
          ) : (
            <>
              <VideoSlot
                label="Variant A"
                side="a"
                url={currentEval.variant_a.url}
                isSelected={winner === 'a'}
                onSelect={() => handleSelectWinner('a')}
                step={votingStep}
                winner={winner}
                onVideoError={handleSkip}
              />
              <VideoSlot
                label="Variant B"
                side="b"
                url={currentEval.variant_b.url}
                isSelected={winner === 'b'}
        ...
        {/* Selection Buttons (Visible in Step 1) */}
        {votingStep === 1 && !isLoading && currentEval && (
          <div className="grid grid-cols-2 gap-10 mb-12">
            <button
              onClick={() => handleSelectWinner('a')}
              className="py-6 rounded-3xl bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest hover:bg-indigo-500/10 hover:border-indigo-500/30 transition-all flex items-center justify-center gap-3 group"
            >
              <Vote className="w-5 h-5 text-indigo-400 group-hover:scale-125 transition-transform" />
              Select Variant A
            </button>
            <button
              onClick={() => handleSelectWinner('b')}
              className="py-6 rounded-3xl bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest hover:bg-purple-500/10 hover:border-purple-500/30 transition-all flex items-center justify-center gap-3 group"
            >
              <Vote className="w-5 h-5 text-purple-400 group-hover:scale-125 transition-transform" />
              Select Variant B
            </button>
          </div>
        )}
            ...
            <div className="animate-in slide-in-from-bottom-8 duration-700">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                  ...
                  <div className="bg-[#0b0e14] border border-white/5 rounded-[40px] p-10 shadow-3xl">
                    <div className="flex items-center justify-between mb-10">
                      ...
                      <div className="text-[10px] font-black uppercase tracking-widest text-indigo-400 bg-indigo-500/5 px-3 py-1.5 rounded-full border border-indigo-500/10">
                        Refining Selection: {winner === 'a' ? 'Variant A' : 'Variant B'}
                      </div>
                    </div>
                        ...
                        <div key={dim.id} className="space-y-4">
                          <div className="flex justify-between items-end">
                            <label className="text-sm font-bold text-gray-300 flex items-center gap-2">
                              <span className={`w-1.5 h-1.5 rounded-full bg-${dim.color}-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]`}></span>
                              {dim.label}
                            </label>
                  ...
                  <div className="space-y-8">
                    <div className="bg-[#0b0e14] border border-white/5 rounded-[40px] p-10 shadow-3xl">
                      ...
                      <textarea
                        ...
                        placeholder="Provide specialized feedback on motion flow, text adherence, or specific model artifacts noticed..."
                        className="w-full bg-[#020408] border border-white/10 rounded-3xl p-6 text-gray-300 placeholder-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/40 transition-all resize-none h-[220px] font-light italic leading-relaxed"
                      />
                    </div>

                  <button
                    onClick={handleSubmitVote}
                    className="w-full bg-gradient-to-r from-indigo-500 via-purple-600 to-pink-600 hover:from-indigo-400 hover:to-pink-500 text-white font-black py-6 rounded-[30px] shadow-[0_20px_50px_rgba(99,102,241,0.3)] transition-all flex items-center justify-center gap-4 group active:scale-[0.98] text-xl"
                  >
                    Deploy Evaluation
                  ...
                  <button
                    ...
                    className="w-full py-4 text-xs font-bold text-gray-500 uppercase tracking-[0.3em] hover:text-gray-300 transition-colors"
                  >
                    ← Re-pick Winner
                  </button>
                </div>
...
function VideoSlot({ label, side, url, isSelected, onSelect, step, winner, onVideoError }: { label: string, side: string, url: string, isSelected: boolean, onSelect: () => void, step: number, winner: string | null, onVideoError?: () => void }) {
  ...
  return (
    <div className={`relative group/video transition-all duration-700 ${isSelected ? 'scale-[1.02]' : step === 2 ? 'grayscale opacity-40 blur-[2px]' : ''}`}>
      ...
      <div className={`relative aspect-video rounded-[36px] overflow-hidden border-2 transition-all duration-500 ${isSelected ? 'border-indigo-500 shadow-3xl' : 'border-white/10 bg-[#0d1017]'}`}>
        <video
          ref={videoRef}
          src={videoUrl}
          loop
          ...
          onError={() => {
            if (retryCount < 1) {
              console.warn(`Video load failed for ${side}, retrying once...`);
              setRetryCount(1);
            } else {
              console.error(`Video load failed for ${side} after retry.`);
              setHasError(true);
              // if (onVideoError) onVideoError(); // Disable auto-skip
            }
          }}
        />
        ...
        {hasError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0d1017] p-8 text-center">
            ...
            <p className="text-[10px] text-gray-500 font-light italic leading-relaxed">
              This variant failed to load from GCS.<br />Please use the "Skip Scenario" button.
            </p>
          </div>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-8 flex items-end justify-between opacity-0 group-hover/video:opacity-100 transition-opacity duration-500">
          <div className="flex items-center gap-4">
            <button
              onClick={togglePlay}
              className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-3xl flex items-center justify-center hover:bg-white/20 transition-all"
            >
              {isPlaying ? <Pause className="w-5 h-5 text-white" /> : <Play className="w-5 h-5 text-white translate-x-0.5" />}
            </button>
            <button
              onClick={toggleMute}
              className={`w-12 h-12 rounded-full backdrop-blur-3xl flex items-center justify-center transition-all ${isMuted ? 'bg-white/10 hover:bg-white/20' : 'bg-indigo-500/40 border border-indigo-500/50'}`}
            >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] font-bold text-gray-500">MAPPED SOURCE_HIDDEN</span>
            </div>
          </div>
          <Award className={`w-8 h-8 ${isSelected ? 'text-indigo-400' : 'text-white/20'}`} />
        </div>

        {/* Big Select Button - REMOVED for decoupling */}
...
function PickButton({ side, onClick }: { side: string, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group relative px-12 py-5 rounded-2xl overflow-hidden transition-all active:scale-95"
    >
...
function NavLink({ icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
  return (
    <a href="#" className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'text-indigo-400' : 'text-gray-500 hover:text-white'}`}>
      {icon} {label}
    </a>
  );
}

function StatsDashboard({ history, ldap, onBack, globalLeaderboard = [] }: { history: any[], ldap: string, onBack: () => void, globalLeaderboard?: any[] }) {
  ...
  // Aggregate stats into model families
  ...
  const buildRadarData = (mode: 't2v' | 'i2v') => {
    const rateKey = mode === 't2v' ? 't2v_rate' : 'i2v_rate';
    const totalKey = mode === 't2v' ? 't2v_total' : 'i2v_total';

    // Sort specific SKUs by this mode's win rate, filter out ones with 0 total
    const sortedSkus = [...skus]
      .filter(s => s[totalKey] > 0)
      .sort((a, b) => b[rateKey] - a[rateKey])
      .slice(0, 3);

    const topModels = sortedSkus.map(s => s.model_id);
    const data: any[] = [];

    DIMENSIONS.forEach(d => {
      const row: any = { subject: d.label };
      sortedSkus.forEach((s: any) => {
        const scores = mode === 't2v' ? s.t2v_scores : s.i2v_scores;
        row[s.model_id] = scores?.[d.id] || 0;
      });
      data.push(row);
    });
    return { data, topModels };
  };
      ...
      <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-lg font-light leading-relaxed">Aggregated performance synthesis for SxS evaluations.</p>
          </div>
          <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
            <button
              onClick={() => setView('global')}
              className={`px-6 py-3 rounded-xl font-bold text-sm uppercase tracking-widest transition-all ${view === 'global' ? 'bg-indigo-500 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
            >
              Global
            </button>
            <button
              onClick={() => setView('user')}
              className={`px-6 py-3 rounded-xl font-bold text-sm uppercase tracking-widest transition-all ${view === 'user' ? 'bg-purple-500 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
            >
              @{ldap}
            </button>
          </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          {models.map(model => (
            <div key={model} className="bg-[#0b0e14] border border-white/5 rounded-[40px] p-8 text-center space-y-4">
              ...
              <div className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-500">
                {Math.round(((familyCounts[model] || 0) / effectiveTotal) * 100)}%
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-white/5 mt-8">
            ...
            <div className="space-y-4">
              {['T2V', 'I2V', 'R2V'].map(mode => (
                <div key={mode} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                  <span className="font-bold text-indigo-400">{mode} Evaluated Pairs</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-light text-gray-300">SKU Specific Win Rates</h3>
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
                {['Global', 'T2V', 'I2V', 'R2V'].map(sm => (
                  <button
                    key={sm}
                    onClick={() => setSkuMode(sm as any)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${skuMode === sm ? 'bg-indigo-500 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                  >
                    {sm}
                  </button>
                ))}
            ...
            <div className="space-y-4 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
              {skus.map((sku: any) => {
                ...
                if (skuMode === 'T2V') {
                  r = sku.t2v_rate; w = sku.t2v_wins; t = sku.t2v_total;
                } else if (skuMode === 'I2V') {
                  r = sku.i2v_rate; w = sku.i2v_wins; t = sku.i2v_total;
                } else if (skuMode === 'R2V') {
                  r = sku.r2v_rate; w = sku.r2v_wins; t = sku.r2v_total;
                }
                ...
                return (
                  <div key={`${sku.model_id}-${skuMode}`} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-indigo-400/70 font-bold uppercase tracking-widest mt-1">Avg Latency: {sku.latency_ps ? `${sku.latency_ps}s / Sec` : 'N/A'}</span>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <span className="font-mono text-lg font-bold block text-white">{r}%</span>
        ...
        <div className="pt-8 border-t border-white/5 mt-8">
          <h3 className="text-xl font-light text-gray-300 mb-6">Dimension Analytics (Spider Charts)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-[#0b0e14] border border-white/5 rounded-[40px] p-6 relative h-[400px]">
              <h4 className="text-center text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Text-to-Video (T2V)</h4>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={t2vRadarData}>
            ...
            <div className="bg-[#0b0e14] border border-white/5 rounded-[40px] p-6 relative h-[400px]">
              <h4 className="text-center text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Image-to-Video (I2V)</h4>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={i2vRadarData}>
                  <PolarGrid stroke="#334155" />
                  <PolarAngleAxis dataKey="subject" tick={{ fill: '#94a3b8', fontSize: 10 }} />
                  <PolarRadiusAxis angle={30} domain={[0, 10]} tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} />
                  <RechartsTooltip contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '12px' }} itemStyle={{ color: '#e2e8f0' }} />
                  <Legend wrapperStyle={{ fontSize: '10px' }} />
                  {i2vTopModels.map((modelId, idx) => (
                    <Radar key={modelId} name={modelId} dataKey={modelId} stroke={CHART_COLORS[idx]} fill={CHART_COLORS[idx]} fillOpacity={0.3} />
                  ))}
                </RadarChart>
              </ResponsiveContainer>
        ...
        <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-[40px] p-10 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-3xl bg-indigo-500 flex items-center justify-center shadow-3xl rotate-3">
              <Crown className="w-10 h-10 text-white" />
            </div>
            <div>
              <h2 className="text-2xl font-bold">Evaluation Tier Unlocked</h2>
              <p className="text-indigo-300/80 font-light">You are now authorized to submit custom benchmarking prompts to the model backlog.</p>
            </div>
          </div>
        ...
        <div className="pt-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              {history.map((h, i) => (
                <div key={i} className="flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                  <div className="flex-1">
                    <div className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1 italic">Prompt Case {total - i}</div>
                    <div className="text-gray-300 font-light line-clamp-1">"{h.prompt}"</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-black text-white uppercase">{h.winner === 'a' ? h.modelA : h.modelB} Won</div>
                    <div className="text-[10px] text-gray-600 font-medium">Score Avg: {(Object.values(h.scores) as number[]).reduce((a, b) => a + b, 0) / 5 || 0}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-xl font-light text-gray-500 mb-8 border-b border-white/5 pb-4 flex items-center gap-3"><Crown className="w-5 h-5 text-yellow-400" /> Top Evaluators Rankings</h3>
            <div className="space-y-4">
              {globalLeaderboard.length > 0 ? globalLeaderboard.map((user, idx) => (
                <div key={user.ldap} className="flex items-center justify-between p-6 bg-[#0b0e14] border border-white/5 rounded-3xl hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-6">
                    <span className={`font-black text-2xl ${idx === 0 ? 'text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]' : idx === 1 ? 'text-gray-300' : idx === 2 ? 'text-amber-600' : 'text-gray-600'}`}>#{idx + 1}</span>
...
function ScenarioReferenceImage({ url, label, onClick }: { url: string, label: string, onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
      className={`w-32 aspect-video rounded-2xl overflow-hidden border border-white/10 shadow-2xl group/img relative bg-[#0d1017] ${onClick ? 'cursor-zoom-in' : ''}`}
    >
      <img
        src={url}
        alt={label}
